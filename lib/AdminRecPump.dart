...
class AdminPumpRec extends StatefulWidget {
  @override
  _AdminPumpRecState createState() => _AdminPumpRecState();
}

class _AdminPumpRecState extends State<AdminPumpRec> {
  int _currentIndex = 0;
  final tabs =[
...
class _ListpageState extends State<Listpage> {
  ...
  Future getPosts() async{
    ...
    QuerySnapshot qn = await firestore.collection("AllRecordDataForSolarPump").get();
    return qn.docs;
  }
            ...
            onPressed: () {
              Navigator.push(
                ...
                MaterialPageRoute(builder: (context) {
                  return Map(
                    latitude: widget.post.get('latitude').toString(),
                    longtiud: widget.post.get('longtiud').toString(),
...
class _Listpage2State extends State<Listpage2> {
  ...
  Future getPosts() async{
    var firestore = FirebaseFirestore.instance;
    QuerySnapshot qn = await firestore.collection("EligibleForSolarPump").get();
    return qn.docs;
  }
  navigateToDetail(DocumentSnapshot post){
...
class _Listpage3State extends State<Listpage3> {
  ...
  Future getPosts() async{
    var firestore = FirebaseFirestore.instance;
    QuerySnapshot qn = await firestore.collection("NotEligibleForSolarPump").get();
    return qn.docs;
  }
  navigateToDetail(DocumentSnapshot post){
    Navigator.push(context, MaterialPageRoute(builder: (context) => Dialogpage2(post: post,)));
